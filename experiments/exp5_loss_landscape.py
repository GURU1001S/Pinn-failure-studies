...
ALPHA_RANGE = (-1.0, 1.0)
SUCCESS_SEED = 0
FAILURE_INIT = "normal_1.0"
N_HESSIAN_RUNS = 5
HESSIAN_N_ITERS = 25
SIGNIFICANCE_RATIO = 0.05
...
def hessian_max_eigenvalue_single(model, seed, n_iters=HESSIAN_N_ITERS,
    ...
    (x_int, t_int), (x_ic, t_ic, u_ic), (x_bc, t_bc, u_bc) =        sample_burgers_domain(3000, 200, 200)
    eigenvalue = 0.0
...
def run_experiment():
    ...
    sharp_fail_mean, sharp_fail_std, samples_fail =        hessian_max_eigenvalue_multiseed(model_fail)
    print(f"\n── Hessian max eigenvalue ({N_HESSIAN_RUNS} runs, Success) ──")
    sharp_success_mean, sharp_success_std, samples_success =        hessian_max_eigenvalue_multiseed(model_success)
