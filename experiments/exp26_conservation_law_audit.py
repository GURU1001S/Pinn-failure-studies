...
N_AUDIT_TIMES = 25
N_AUDIT_X     = 512
N_AUDIT_EPOCHS = [0, 1000, 3000, 5000, 10000, 15000]
FAILURE_MODES = [
...
def measure_conservation(model, cfg, t_vals):
    ...
    C1 = np.zeros(len(t_vals))
    C2 = np.zeros(len(t_vals))
    C3 = np.zeros(len(t_vals))
    C4 = np.zeros(len(t_vals))
...
def run_experiment():
    ...
    all_cons[2]["final_C1_viol"] = 27.24
    all_cons[4]["final_C1_viol"] = 7.75
