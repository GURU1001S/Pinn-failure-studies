"""
exp18_compound_failure.py — Compound Failure Interaction Map
[v2 — journal-ready fixes]

Novel two-failure interaction study combining spectral bias + gradient pathology
simultaneously on the 1D advection equation at β=30 (spectral failure zone).

Protocol:
  1. Train for 50,000 iterations, recording at TRACK_EVERY steps:
     (a) Spectral error per frequency band (FIX 1: corrected metric)
     (b) Gradient magnitude ratio |∇L_pde| / |∇L_bc|
     (c) Per-component loss values
  2. At NTK checkpoints compute eigenvalue spectrum (κ, decay exponent)
  3. Analyse correlation between failure modes
  4. Generate joint phase portrait

Outputs (results/exp18/):
  - spectral_error_trajectories.png
  - gradient_ratio_trajectory.png
  - ntk_spectra_checkpoints.png
  - joint_phase_portrait.png
  - correlation_analysis.png
  - exp18_results.json

FIXES vs v1 (journal-ready):
  [FIX 1] Spectral error metric corrected — v1 computed:
            err_fft[:, k].mean() / (exact_fft[:, k].mean() + 1e-10)
          For the advection solution u=sin(x-βt), exact_fft[:, k]≈0
          for k≥2 (energy only at k=1). So the denominator collapses
          to the guard 1e-10, giving values like 1.7×10¹⁰ for all k≥2.
          The heatmap appeared entirely black; the correlation analysis
          used noise/epsilon, not spectral error.
          v2 uses a per-band RELATIVE L2 norm:
            ||err_field filtered to band k||₂ / ||u_exact||₂
          by masking the FFT to band k only and inverse-transforming.
          This gives physically meaningful values in [0, ~2].

  [FIX 2] Honest correlation reporting — v1 reported Pearson r=0.082
          (essentially zero) but also a cross-correlation peak of 0.976,
          creating a contradictory impression. With broken spectral
          measurement, both numbers were invalid. v2 reports corrected
          Pearson r on corrected spectral errors, adds a significance
          test (bootstrap p-value), and explicitly flags when the two
          failure modes are NOT significantly correlated.

  [FIX 3] NTK condition number jump annotated — κ decreased from 7.4e9
          at iter=0 to 5.1e9 at iter=30k, then jumped to 3.7e10 at
          iter=50k. v2 detects this non-monotone jump and annotates it
          on ntk_spectra_checkpoints.png with a note about potential
          numerical instability at the final checkpoint.

  [FIX 4] Explicit seed=42 for reproducibility.

  [FIX 5] Spectral heatmap colormap — v1 vmax was dominated by the
          10¹⁰ noise values (black everywhere). v2 clips at 99th
          percentile so the actual variation in spectral error is
          visible.
"""
...
# ===================================================================
# FIX 1 — Corrected spectral error metric
# ===================================================================

def spectral_error_per_band(model, n_freq=N_FREQ_BANDS,
                             nx=NX_EVAL, nt=NT_EVAL):
    """
    For each frequency band k, compute:
        ||IFFT(FFT(err_field) * mask_k)||_2 / ||u_exact||_2

    where mask_k zeroes all FFT bins except k and (nx-k) (symmetry).

    FIX 1: v1 computed err_fft[:,k] / (exact_fft[:,k] + 1e-10).
    For u=sin(x-βt), exact_fft[:,k]≈0 for k≥2, so the denominator
    collapses to 1e-10 and values explode to ~1e10. v2 normalizes by
    the global ||u_exact||_2, giving physically bounded values in [0,~2].
    """
    ...
    u_ex      = exact(XX, TT)
    err_field = u_pred - u_ex                   # (nt, nx)
    global_norm = np.linalg.norm(u_ex) + 1e-10  # scalar

    err_fft = np.fft.rfft(err_field, axis=1)    # (nt, nx//2+1)

    band_errors = []
    for k in range(1, n_freq + 1):
        # FIX 1: zero all bins except k, inverse transform, measure norm
        masked       = np.zeros_like(err_fft)
        masked[:, k] = err_fft[:, k]
        err_k_spatial = np.fft.irfft(masked, n=nx, axis=1)  # (nt, nx)
...
def bootstrap_pvalue(a, b, n_boot=1000, seed=0):
    """Two-tailed p-value for Pearson r via permutation bootstrap."""
...
def plot_spectral_trajectories(history, filepath):
    """FIX 5: clip colormap at 99th pct so variation is visible."""
    steps_rec = history["steps_recorded"]
    spec_arr  = np.array(history["spectral_errors"])   # (n_rec, n_freq)
    ...
    ax = axes[0]
    # FIX 5: shared colormap clipped at 99th percentile
...
def plot_ntk_spectra(ntk_data, filepath):
    """FIX 3: annotate non-monotone condition number jump."""
...
def plot_correlation_analysis(history, r_sg, pval_sg,
                               ...
                               lag_sg, xcorr_peak,
                               filepath):
    """FIX 2: include p-values and explicit significance flags."""
    steps     = np.array(history["steps_recorded"])
    spec_mean = np.array(history["spectral_errors"]).mean(axis=1)
...
def run_experiment():
    ...
    if start_epoch <= N_EPOCHS:
        # Check if there are any NTK checkpoints <= completed_epoch not yet in ntk_data
        ...
        for epoch in range(start_epoch, N_EPOCHS + 1):
            ...
            if epoch % TRACK_EVERY == 0:
                spec_err = spectral_error_per_band(model)  # FIX 1
