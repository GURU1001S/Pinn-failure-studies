"""
exp8_collocation_starvation.py — Collocation Count & Adaptive Refinement
[v2 — journal-ready fixes]

Helmholtz PINN trained with collocation counts:
  [50, 100, 200, 500, 1000, 2000, 5000]
Each count tested with N_SWEEP_SEEDS seeds (mean ± std reported).
Finds minimum viable count. Then runs adaptive refinement:
  Start 500 pts (above starvation cliff) → pre-train to convergence
  → identify top-20% residual regions → add 100 more → repeat × 10.

Outputs (results/exp8/):
  - l2_vs_count.pdf           (error bars, silent failure annotation)
  - convergence_curves.pdf    (silent failure annotation added)
  - adaptive_vs_uniform.pdf   (fixed: starts above cliff, proper LR)
  - exp8_results.json

FIXES vs v1 (journal-ready):
  [FIX 1] Adaptive refinement completely rewritten:
          v1 started from 100 points (BELOW the starvation cliff),
          causing the base model to be completely wrong before any
          refinement. Adaptive selection then used a failed model to
          identify "high-residual regions" — but every region has
          high residual when the model is wrong, making selection
          effectively random. Result: adaptive L2 rose from 2.5 to
          3.4 over 10 rounds — the experiment was measuring a bug,
          not adaptive refinement.
          v2 starts from 500 points (above the cliff), pre-trains
          for full N_PRETRAIN_EPOCHS to reasonable convergence before
          any adaptive round begins, then refines.

  [FIX 2] LR scheduler reset between adaptive rounds:
          v1 created a new CosineAnnealingLR each round starting from
          whatever LR the optimizer had decayed to (near lr_min=1e-5
          from round 2 onward), preventing meaningful training.
          v2 explicitly resets the optimizer LR to LR_ADAPTIVE at
          the start of each round before creating the new scheduler.

  [FIX 3] Multi-seed count sweep:
          v1 ran each count once (single seed) — the minimum viable
          count claim "N=1000" rested on one run per count.
          v2 runs N_SWEEP_SEEDS=3 seeds per count and reports
          mean ± std. Error bars added to l2_vs_count bar chart.
          Non-monotonicity at N=200 investigated across seeds.

  [FIX 4] Silent failure annotation on convergence_curves.pdf:
          v1 showed N=50/100 achieving lower training loss than N=1000
          without annotation. This is the most important diagnostic
          finding — the network achieves low loss on sparse points
          while completely failing on the full domain. v2 adds an
          explicit text box and annotation arrows.

  [FIX 5] non_monotonicity_note in JSON for N=200 result and
          uncertainty qualification of the minimum viable count.
"""
...
N_HIDDEN   = 4
N_NEURONS  = 64
N_EPOCHS   = 10000          # count sweep
N_SWEEP_SEEDS = 3           # FIX 3: seeds per count
N_BC       = 200

COLLOCATION_COUNTS = [50, 100, 200, 500, 1000, 2000, 5000]
L2_FAILURE_THRESHOLD = 0.10

# FIX 1: adaptive starts above cliff
ADAPTIVE_INIT_POINTS  = 500     # was 100 — now above starvation cliff
ADAPTIVE_ADD_POINTS   = 100
ADAPTIVE_ROUNDS       = 10
N_PRETRAIN_EPOCHS     = 5000    # pre-train base model to convergence first
EPOCHS_PER_ROUND      = 2000    # FIX 2: more epochs per round
LR_ADAPTIVE           = 5e-4    # FIX 2: restart LR each round
...
def train_helmholtz_custom(model, n_int, n_bc=N_BC,
                            ...
                            return_loss=True):
    """Train Helmholtz PINN with fixed collocation count."""
...
# ===================================================================
# FIX 1 + FIX 2 — Corrected adaptive refinement
# ===================================================================

def adaptive_refinement_step(model, x1_pts, x2_pts,
                              n_add=ADAPTIVE_ADD_POINTS, top_frac=0.2):
    """
    Identify high-residual regions using a CONVERGED model
    and add points there.

    FIX 1: only called after model has been pre-trained to convergence,
    ensuring residual estimates are meaningful (not noise from a
    catastrophically failed model).
    """
    model.eval()
...
def train_one_round(model, optimizer, x1_int, x2_int,
    ...
    """
    Train for one adaptive round.
    FIX 2: LR is reset externally before each call so the scheduler
    always starts from LR_ADAPTIVE, not from a decayed value.
    """
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=n_epochs, eta_min=1e-5)
...
def run_adaptive(n_rounds=ADAPTIVE_ROUNDS,
                 ...
                 seed=0):
    """
    FIX 1: start above starvation cliff (500 pts).
    FIX 2: reset LR each round.
    Pre-train to convergence before any adaptive refinement.
    """
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    (x1_int, x2_int), (x1_bc, x2_bc, u_bc) = \
        sample_helmholtz_domain(init_pts, N_BC)
    ...
    for rd in range(n_rounds):
        # FIX 2: reset optimizer LR before each round
...
def run_uniform(n_rounds=ADAPTIVE_ROUNDS,
                ...
                seed=0):
    """
    Uniform growth control — same budget as adaptive.
    FIX 2: same LR reset pattern for fair comparison.
    """
    torch.manual_seed(seed)
    np.random.seed(seed)
...
# ===================================================================
# FIX 4 — Convergence curves with silent failure annotation
# ===================================================================

def plot_convergence_annotated(count_results, counts, filepath):
    """
    Training convergence by collocation count with silent failure
    annotation.

    FIX 4: v1 showed N=50/100 achieving lower training loss than
    N=1000 without any annotation. This is the most important
    diagnostic finding — low training loss does NOT imply correct
    solution when the collocation point density is below the
    starvation cliff.
    """
    ...
    for i, n in enumerate(counts):
        ...
        # Use first seed's loss history for the curve
    ...
    # FIX 4: silent failure annotation
    ...
    # Mark starvation cliff
    ax.axvline(0, alpha=0)   # dummy to force y-axis
...
# ===================================================================
# FIX 3 — L2 vs count with error bars
# ===================================================================
...
def plot_l2_vs_count_errorbars(count_results, counts,
                                ...
                                filepath):
    """
    L2 vs count bar chart with mean ± std error bars.
    FIX 3: v1 showed single-seed values with no uncertainty.
    v2 shows mean ± std from N_SWEEP_SEEDS runs.
    """
    ...
    # Annotate N=200 non-monotonicity if present
    ...
    if n200_idx is not None:
        ...
        if n200_mean > n100_mean * 1.2:   # significant non-monotonicity
            ax.annotate(
                "Non-monotonic:\nN=200 > N=100\n(see notes)",
...
def run_experiment():
    ...
    # ── Part 1: Multi-seed count sweep (FIX 3) ─────────────────────
    ...
    # Find minimum viable count (based on mean L2)
    min_viable = None
    ...
    # ── Part 2: Adaptive vs Uniform (FIX 1 + FIX 2) ────────────────
    ...
    # FIX 3: error bars on l2_vs_count
    ...
    # FIX 4: annotated convergence curves
    plot_convergence_annotated(
        count_results, COLLOCATION_COUNTS,
        filepath=OUTPUT_DIR / "convergence_curves.pdf")

    # Adaptive vs uniform (now with fixed implementation)
    ...
    results = {
        ...
        # FIX 5: non-monotonicity documentation
