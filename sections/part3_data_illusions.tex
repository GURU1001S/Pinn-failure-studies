...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\linewidth]{results/exp8/l2_vs_count.pdf}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\linewidth]{results/exp8/convergence_curves.pdf}
...
The extrapolation horizon is effectively non-existent. Across all tested training intervals, the network completely collapses immediately past the temporal boundary, routinely yielding $R^* \le 1.26$. Attempts to fit the out-of-domain error trajectory to established continuous growth models (linear, polynomial, and exponential) uniformly failed, with all models yielding $R^2 < 0.36$ for $T_{train}=1.0$. The error does not follow any continuous growth law beyond $T_{train}$; it saturates immediately at values consistent with the trivial attractor, suggesting the model does not extrapolate but rather snaps to its failure mode. The structural stiffness explicitly confines the network's predictive capabilities strictly to the interpolated convex hull of its collocation data.
